...
nx = (int)(Nx_aprox / dy )
ny = (int)(Ny_aprox / dy )
parameters['Nx'] = Nx_aprox
parameters["Ny"] = Ny_aprox
...
mesh = fe.RectangleMesh( Point(0, 0), Point(Nx, Ny), nx, ny)

# Printing Initial Mesh Informations 

# Calculate the number of cells in each mesh across all processes
number_of_coarse_mesh_cells = df.MPI.sum(comm, mesh.num_cells())
...
if rank == 0:
    ...
    min_dx_coarse = mesh.hmin() / df.sqrt(2)
